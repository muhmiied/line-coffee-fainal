...
export default function ProductDetailClient({ product, category, relatedProducts = [] }: ProductDetailClientProps) {
  ...
  const [selectedWeight, setSelectedWeight] = useState<WeightLabel>(
    () => (product.sizes[0]?.label ?? "250g") as WeightLabel,
  );
  ...
  const selectedPrice = priceOptions.find((option) => option.label === selectedWeight) ?? priceOptions[0] ?? { label: "250g" as WeightLabel, price: 0 };
