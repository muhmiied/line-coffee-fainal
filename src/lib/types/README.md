...
| `customer.ts` | Customer / Address / Snapshots | **LIVE (partial)** | Live shapes: `customer-account.ts` (`CustomerProfile`, `CustomerAddress`, …) for the public account area; `admin-orders.ts` snapshots; Phase 12A `admin-customers.ts` (`AdminCustomerSummary`/`AdminCustomerDetail`/`AdminCustomerAddress`) reads `customers` + `customer_addresses` directly for the Admin Customers module (grants: `20260703130000_phase12a_admin_customers_read_grants.sql`) |
| `inventory.ts` | Inventory items / movements / suppliers / lots / allocations / packaging / beans | **LIVE (partial)** | Finished-product `InventoryLot` + `OrderLotAllocation` map through `admin-purchasing.ts`. Phase-6 `PackagingItem`, `PackagingMovement`, and `PackagingRequirement` map through `admin-packaging.ts`; packaging remains count-based and separate from coffee FIFO. Raw-bean inventory: `EspressoBeanStock` here is an older unified dormant shape — the **real, live** Phase-8 raw-bean lot/allocation contract is `builders.ts` (`EspressoBeanAllocation`) + `admin-espresso.ts` (`AdminEspressoBean`), backed by `espresso_beans` / `espresso_bean_stock` / `espresso_bean_lots` / `order_espresso_bean_allocations`. |
...
| `cms.ts` | Reviews / blog / contact / legal | **LIVE** | Phase 13A tables/RLS/RPCs; live data layers in `admin/admin-cms.ts` and `cms/public-cms.ts` |
| `settings.ts` | Site settings | **FORWARD** | Owner Settings UI deferred (master plan §6.1) |
| `flavor.ts` | Make-Your-Flavor config (older speculative shape) | **SUPERSEDED** | Predates the real Phase 9 catalog; vocabulary does not match `flavor_bases`/`flavor_items`. Use `builders.ts` + `admin-flavor.ts` instead |
| `builders.ts` | Phase 8/9 custom-builder checkout payloads + order snapshots | **LIVE** | `EspressoBuilderPayload`/`FlavorBuilderPayload` → `CartItem.customData` (`cart.tsx`) and the `create_checkout_order` RPC item shape (checkout page). `EspressoBeanAllocation` → `admin-espresso.ts`. Server-side catalog + FIFO: migration `20260701120000_phase8_9_espresso_flavor_builders.sql` |
