...
| `inventory.ts` | Inventory items / movements / suppliers / **lots** / beans | **LIVE (partial)** | `InventoryLot` (Phase 4) → live shape in `admin-purchasing.ts` (`inventory_lots` table). `Supplier` → live shape `admin-purchasing.ts` (`suppliers`). Generic `InventoryItem`, raw-bean/packaging lots still **FORWARD** (Phases 5–6, 8). Live operational stock = `inventory_stock` + `inventory_movements`, kg-per-product (Phase 1) |
