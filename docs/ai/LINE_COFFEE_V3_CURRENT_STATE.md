...
- The phased plan (what to build next, in order) lives in `LINE_COFFEE_V3_MASTER_EXECUTION_PLAN.md` (canonical). **Current position:** Phases 1–11, Phase 12A (Admin Customers), Phase 13A (CMS real data), Phase 14A (Admin Dashboard real numbers), Phase 15/15B–15D (Admin Accounting real financials + expense/purchase/supplier-payment writes), Phase 16A (Admin Analytics real reports), Phases 17A–17B (real admin settings + safe public settings wiring), Phase 18A (Telegram order notification + WhatsApp handoff), and Phase 18B (store-closed checkout enforcement + durable Telegram dedup log) are applied. Broad mock Inventory/Marketing/Espresso-Manager/Flavor-Manager UI wiring remains deferred; order item/price editing is deliberately deferred as unsafe without RPC ownership. Product-image follow-up and behavioural web-traffic tracking remain unstarted.
