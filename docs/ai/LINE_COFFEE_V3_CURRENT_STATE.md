...
- The phased plan (what to build next, in order) lives in `LINE_COFFEE_V3_MASTER_EXECUTION_PLAN.md` (canonical). **Current position:** Phases 1–11, Phase 12A (Admin Customers), Phase 13A (CMS real data), Phase 14A (Admin Dashboard real numbers), Phase 15/15B–15D (Admin Accounting real financials + expense/purchase/supplier-payment writes), Phase 16A (Admin Analytics real reports), Phases 17A–17B (real admin settings + safe public settings wiring), Phase 18A (Telegram order notification + WhatsApp handoff), Phase 18B (store-closed checkout enforcement + durable Telegram dedup log), Phase 19A (product images + Supabase Storage), Phase 19B (real packaging stock admin tab), and Phase 19C (real promo-code admin tab + checkout promo clarity) are applied. Coffee/bean/supplier Inventory views, Marketing offers/announcements/performance, and Espresso-Manager/Flavor-Manager remain mock; order item/price editing is deliberately deferred as unsafe without RPC ownership. Behavioural web-traffic tracking remains unstarted.
