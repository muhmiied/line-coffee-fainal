...
| **Admin + public settings** (Phases 17A–17B) | `site_settings` stores real public brand/contact/social/storefront values. Admin can edit the four public keys; the public header, footer, contact page, and checkout notice read only those public-scoped rows through anon/publishable access. Missing/invalid contact and social values are hidden or replaced by an honest unavailable state. Delivery pricing remains owned by `resolve_delivery_fee()`. | `src/lib/admin/admin-settings.ts`; `/admin/settings`; `PublicHeader`; `PublicFooter`; `/contact`; `/checkout` |
...
| **Product images** (Phase 19A) | Real product images in the public **`product-images`** Supabase Storage bucket (public read, admin-only write via `is_admin()` storage policies). Admin ProductDrawer → Media tab uploads / previews / sets-primary / deletes images, writing storage public URLs into the existing `products.image_url` (primary) + `products.gallery` (jsonb) — no new table. Public product cards, detail page, and category page render the primary image (unchanged code) with the existing static fallback when a product has none. `next.config.ts` allows the Supabase public-storage image host. | migration `20260704180000`; `src/lib/admin/admin-product-images.ts`; `src/components/admin/products/ProductDrawer.tsx`; `next.config.ts` |
...
**Phase 8-9 foundation (applied):** migration `20260701120000_phase8_9_espresso_flavor_builders.sql` adds a raw-bean catalog with a separate kg-based FIFO inventory dimension plus a cost-only flavor catalog (no flavor stock table — Decision 4). `create_checkout_order` accepts and server-prices `custom_espresso`/`custom_flavor`, validates exact espresso ratios, reserves bean lots, and writes cost-free snapshots. `update_admin_order_status` deducts/release bean reservations and rolls flavor cost snapshots into delivered COGS. Packaging remains product-only by documented deferral. Data layer: `src/lib/types/builders.ts`, `src/lib/admin/admin-espresso.ts`, `src/lib/admin/admin-flavor.ts`; broad admin manager wiring is deferred.
...
- The phased plan (what to build next, in order) lives in `LINE_COFFEE_V3_MASTER_EXECUTION_PLAN.md` (canonical). **Current position:** Phases 1–11, Phase 12A (Admin Customers), Phase 13A (CMS real data), Phase 14A (Admin Dashboard real numbers), Phase 15/15B–15D (Admin Accounting real financials + expense/purchase/supplier-payment writes), Phase 16A (Admin Analytics real reports), Phases 17A–17B (real admin settings + safe public settings wiring), Phase 18A (Telegram order notification + WhatsApp handoff), Phase 18B (store-closed checkout enforcement + durable Telegram dedup log), and Phase 19A (product images + Supabase Storage) are applied. Broad mock Inventory/Marketing/Espresso-Manager/Flavor-Manager UI wiring remains deferred; order item/price editing is deliberately deferred as unsafe without RPC ownership. Behavioural web-traffic tracking remains unstarted.
