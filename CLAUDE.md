...
- **MISSING (no DB):** order_item_components · raw-bean inventory · refunds/returns · reviews · contact_messages · analytics. Packaging + promo/pricing are authored in Phase-6/7 migration `20260701104031` but are not live until an owner-approved `db push`. **Media Studio does not exist and is cancelled (Decision 1).**
...
**Position:** Phases 1–2 are applied. Phases 3–7 are implemented in code/docs; Phase 4–7 migrations are authored only and **not applied**. Phase-6/7 migration `20260701104031` preserves the Phase-5 coffee FIFO function internally and adds separate packaging items/lots/movements/order lines, Place-Order deduction with non-blocking shortage flags, FIFO packaging cost snapshots, promo codes/redemptions, server-side product-subtotal pricing, and retry-safe checkout integration. Minimal checkout promo UI + typed admin packaging/marketing data layers are included; broad mock admin UI wiring is deferred. **Phase 8 is next and has not started.**
